"""
Created on Tuesday 13 January 2026, 12:11:22
...
def create_Overall_Remediation_Across_Schemes2(type, figure_count, colours, paths_variables, data_label_font_dict_white, data_label_font_dict_black):
    ###########
    # Main script Notifications
    if type==0:
        text = f'Figure{figure_count}_Overall_Remediation_Across_Schemes2'
        print(text)

    if type==1:
        text = f'Accessible_Figure{figure_count}_Overall_Remediation_Across_Schemes2'
        print(text)
    ###########
    ...
    # Accessing, transforming and obtaining values from Social_1b
    Social_1 = pd.read_excel(MI_tables_path, sheet_name='Social_1')
    Social_1b = chop_df(Social_1, 10, 6)
    Social_complete = Social_1b.iloc[0, 6] 
    Social_underway = Social_1b.iloc[1, 6]
    Social_in_programme = Social_1b.iloc[2, 6] + Social_1b.iloc[3, 6]
    ...
    }, index=["Social self-funded", "Developer", "CSS", "BSF", "ACM"])
